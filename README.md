...
**Discagem** — `dial-up-sound_1.mp3`, tocado **uma vez, no login**, quando o agente
entra no sistema. O arquivo tem 27,1 s; o jogo detecta sozinho o início real do áudio
(1,70 s de silêncio de cabeça) e pula essa parte, resultando em **25,4 s** de modem
a partir do instante em que o desktop aparece. Se um trace começar durante a discagem,
ela some num *fade* para não abafar o alarme.

Carregado por três caminhos, nesta ordem: buffer decodificado (integra-se ao grafo de
áudio, então mudo e *fade* funcionam), elemento `<audio>` (o único que funciona ao abrir
o `index.html` direto, onde `fetch` é bloqueado por CORS) e, se nada carregar, silêncio.
...
dial-up-sound_1.mp3    gravação da discagem, tocada no login
